...
plot_re <- function(re_df, x_val){
  ...
  figure <- ggplot(mean_perf_re_df, aes(x = Index, y = MeanRelativeError)) +
    ...
    coord_cartesian(ylim = c(-x_val*2, x_val*2)) +
...
grid_plot <- function(re_list, x_axis_lab, x_val){
  library(ggplot2)
  ...
  x_axis_label_plot <- ggplot() +
    geom_text(aes(x = 1, y = 1, label = x_axis_lab), size = 8, color = "black", angle = 270) +
    theme_void() +
    theme(plot.margin = margin(0, 0, 0, 0))
  ...
  x_axis_plot <- ggplot() +
    ...
    coord_cartesian(ylim = c(-x_val*2, x_val*2))+
    geom_text(aes(x = 0.5, y = x_val, label = paste0(" ", x_val)), size = 5, color = "black")+
    geom_text(aes(x = 0.5, y = -x_val, label = paste0("-", x_val)), size = 5, color = "black")+
    geom_text(aes(x = 0, y = x_val, label = "-"), size = 8, color = "black", hjust = 0, vjust = 0.34)+
  ...
  scenario_1_plot <- plot_re(re_list[[1]], x_val)
  scenario_2_plot <- plot_re(re_list[[2]], x_val)
  scenario_3_plot <- plot_re(re_list[[3]], x_val)
  scenario_4_plot <- plot_re(re_list[[4]], x_val)
  scenario_5_plot <- plot_re(re_list[[5]], x_val)
  scenario_6_plot <- plot_re(re_list[[6]], x_val)
  scenario_7_plot <- plot_re(re_list[[7]], x_val)
  scenario_8_plot <- plot_re(re_list[[8]], x_val)
  scenario_9_plot <- plot_re(re_list[[9]], x_val)
  scenario_10_plot <- plot_re(re_list[[10]], x_val)
  scenario_11_plot <- plot_re(re_list[[11]], x_val)
  scenario_12_plot <- plot_re(re_list[[12]], x_val)
  scenario_13_plot <- plot_re(re_list[[13]], x_val)
  scenario_14_plot <- plot_re(re_list[[14]], x_val)
  scenario_15_plot <- plot_re(re_list[[15]], x_val)
  scenario_16_plot <- plot_re(re_list[[16]], x_val)
