...
#############################################################
#TMB SCAAs fit to Fishery data, with DM for Derek
#############################################################
...
compile("SCAA_forDerek_wAE_wDM.cpp")
...
res_list<-list()
  for (s in N_sim){
    ...
    par <- list(log_M=log(runif(1,min=OM$OM$Mref-OM$OM$Mref*0.35,max=OM$OM$Mref+OM$OM$Mref*0.35)),
                ...
                ltheta=log(runif(1,min=5-0.35*5,max=5+0.35*5)),
    ...
    #TMB stuff
    ################
    dyn.load(dynlib("SCAA_forDerek_wAE_wDM"))
    
    parm_names<-names(MakeADFun(dat, par, DLL="SCAA_forDerek_wAE_wDM")$par)
    
    fixed<-list(steepness=factor(NA),
                log_sd_catch=factor(NA),
                log_sd_index=factor(NA))
    
    lower_bounds<-c(-5,-20,rep(-10,dat$lage),rep(-10,dat$lyear), 0, 5, -5,-5,-5,-5,-5,-5,rep(-10,dat$lyear))
    upper_bounds<-c( 2,  1,rep( 10,dat$lage),rep( 10,dat$lyear), 1, 25, 2, 2, 2, 5, 5,10,rep(  0,dat$lyear))
    ...
    SCAA <- MakeADFun(dat, par, DLL="SCAA_forDerek_wAE_wDM", map=fixed, random=reffects)
