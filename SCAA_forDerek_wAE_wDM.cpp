...
//back-transform log-scale params
  ...
  Type theta=exp(ltheta);                         //back-transform
  ...
  //Linear Version from Thorson
   if(SS_fishery(i)>0){
    for(j=fage;j<=lage;j++){
     ...
     DM_component_2(i,j)=lgamma(SS_fishery(i)*obs_fishery_comp(i,j)+Beta(i)*pred_fishery_comp_wAE(i,j))-lgamma(Beta(i)*pred_fishery_comp_wAE(i,j));
    }
